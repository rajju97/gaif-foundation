...
const SellerOrders = () => {
    ...
    // ⚡ Bolt Optimization: Replace multiple `.filter().length` calls with a single O(N) reduce pass.
    // Impact: Reduces iterations over the orders array from 5x to 1x and caches the result.
        ...
        <div className="container mx-auto p-4 max-w-5xl">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
                {['all', 'pending', 'confirmed', 'shipped', 'delivered', 'cancelled'].map((status) => (
                    ...
                    >
                        ...
                        {status !== 'all' && (
                            <span className="ml-1 badge badge-sm">{statusCounts[status] || 0}</span>
                        )}
                    </button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-ds text-center">
                    <p className="text-2xl font-bold text-blue-600">{statusCounts.confirmed || 0}</p>
