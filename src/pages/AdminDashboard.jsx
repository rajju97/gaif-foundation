...
const AdminDashboard = () => {
  ...
  // Consolidated 3 separate O(N) filter/reduce passes over the `orders` array
  // into a single O(N) reduce operation, wrapped in useMemo to prevent
  // recalculation on every re-render unless `orders` changes.
  const stats = useMemo(() => {
    return orders.reduce((acc, o) => {
      if (o.status === 'delivered') {
        acc.totalRevenue += (o.total || 0);
        acc.commissionAmount += (o.commissionAmount || 0);
        acc.gstAmount += (o.gstAmount || 0);
      }
      return acc;
    }, { totalRevenue: 0, commissionAmount: 0, gstAmount: 0 });
  }, [orders]);

  const totalRevenue = stats.totalRevenue;

  // ⚡ Bolt Performance Optimization:
  // Memoized sellers filter to prevent recalculation on unrelated re-renders.
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-7 gap-4 mb-6">
        ...
        <div className="bg-base-100 p-4 rounded-lg shadow text-center">
          <p className="text-2xl font-bold text-orange-600">&#8377;{stats.commissionAmount.toFixed(0)}</p>
          <p className="text-sm text-gray-500">Commission</p>
        </div>
        <div className="bg-base-100 p-4 rounded-lg shadow text-center">
          <p className="text-2xl font-bold text-teal-600">&#8377;{stats.gstAmount.toFixed(0)}</p>
