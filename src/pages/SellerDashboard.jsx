...
const SellerDashboard = () => {
  ...
  // Stats
  // Bolt Optimization: Consolidate multiple O(N) filter.reduce and filter.length iterations
  // into a single O(N) reduce pass, memoizing the result to prevent unnecessary recalculation.
  const stats = useMemo(() => {
    return orders.reduce((acc, o) => {
      if (o.status === 'delivered') {
        acc.totalRevenue += (o.sellerEarning ?? o.total ?? 0);
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-wrap justify-between items-center mb-6">
        ...
        <button onClick={() => navigate('/seller-orders')} className="btn btn-primary btn-sm">
          ...
          {stats.pendingOrders > 0 && <span className="badge badge-warning ml-2">{stats.pendingOrders}</span>}
        </button>
      </div>
